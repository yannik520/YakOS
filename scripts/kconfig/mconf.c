...
"between different YakOS configurations.\n"
"\n"
...
"------------------------------\n"
"If you prefer to have all of the YakOS options listed in a single\n"
...
load_config_help[] = N_(
	...
	"YakOS's default, entering the name of the file here will allow you\n"
...
static char buf[8192*20], *bufptr = buf;
...
int main(int ac, char **av)
{
	...
	sym_calc_value(sym);
	sprintf(menu_backtitle, _("YakOS %s Configuration"),
		sym_get_string_value(sym));
	...
	do {
		cprint_init();
		cprint("--yesno");
		cprint(_("Do you wish to save your new YakOS configuration?"));
		cprint("5");
		cprint("60");
		stat = exec_conf();
	} while (stat < 0);
	...
	if (stat == 0) {
		if (conf_write(NULL)) {
			fprintf(stderr, _("\n\n"
				"Error during writing of the YakOS configuration.\n"
				"Your YakOS configuration changes were NOT saved."
				"\n\n"));
			return 1;
		}
		printf(_("\n\n"
			"*** End of YakOS configuration.\n"
			"*** Execute 'make' to build YakOS."
			"\n\n"));
	} else {
		fprintf(stderr, _("\n\n"
			"Your YakOS configuration changes were NOT saved."
